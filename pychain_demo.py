...
if st.button("Generate Private and Public Keys"):
    generate_keys()
    st.write("Private and public keys generated and saved to files!")
...
def generate_keys(otp_secret=None):
    # Generate a new RSA key pair
    private_key = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    ...
    with open("private_key.pem", "wb") as f:
        f.write(pem_private_key)
        st.warning("Make sure to save your private key in a secure location! Losing your private key may compromise all your future transactions on the network.")

    with open("public_key.pem", "wb") as f:
        f.write(pem_public_key)

    # If OTP secret is provided, generate and display a QR code for the user to scan
    if otp_secret:
        totp = pyotp.TOTP(otp_secret)
        qr_code = totp.provisioning_uri(name="MyApp")
        st.image(qr_code)

def authenticate(otp_secret):
